...
//        // Insert Other Transactions
    ...
    public static void Verification(){
        for (Account data: (ArrayList<Account>) DatabaseHelper.get(Account.getQuery(), Account.TABLE_NAME)){
            Log.d("fuck", data.toString());
        }
        for (Customer data: (ArrayList<Customer>) DatabaseHelper.get(Customer.getQuery(), Customer.TABLE_NAME)){
            Log.d("fuck", data.toString());
        }
        for (Owns data: (ArrayList<Owns>) DatabaseHelper.get(Owns.getQuery(), Owns.TABLE_NAME)){
            Log.d("fuck", data.toString());
        }
        for (Transaction data: (ArrayList<Transaction>) DatabaseHelper.get(Transaction.getQuery(), Transaction.TABLE_NAME)){
            Log.d("fuck", data.toString());
        }
    }
}
